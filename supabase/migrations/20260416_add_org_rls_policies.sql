...
-- SAFE APPROACH: Drop only if exists, then create (using DO block for safety)

DO $$
BEGIN
  -- Drop the policy if it exists (safe - won't error if missing)
  DROP POLICY IF EXISTS "Org members can view org provider appointments" ON public.appointments;
...
-- Create indexes for performance on org_workers lookup
...
-- Document the policy for reference
COMMENT ON POLICY "Org members can view org provider appointments" ON public.appointments IS
  'SECURITY POLICY: Allows org team members to see appointments where any team member is provider. 
