...
### Dallas-specific note — 2026-05-07
- My Appointments.tsx changes broke the app despite tsc passing
- The file has known fragility: large, import-sensitive, silent blank-page crashes
- Build gate (tsc) is necessary but NOT sufficient — runtime testing is also required
- Next attempt at cash button: make the smallest possible change, verify runtime before commit

### 2026-05-07 — Session: Cash button visibility (safe approach, tsc passed)

**What changed:**
- Created `supabase/migrations/20260507000001_add_payment_method_type_to_proofs.sql` — adds `payment_method_type text` column
- Updated `src/integrations/supabase/types.ts` — added `payment_method_type: string | null` to Row/Insert/Update for payment_proofs
- `src/components/Appointments.tsx` surgical changes only:
  - Added `paymentMethodType` state (string, '')
  - Updated bulk select query to include `payment_method_type`
  - Added `cashAppointmentIds` useMemo (Set<string>) after `paidAppointmentIds` — paidAppointmentIds left untouched
  - Updated upsert to include `payment_method_type: paymentMethodType || null`
  - Added Cash/Other payment method toggle buttons in proof submission form
  - Both Paid buttons (pending ~line 892, confirmed ~line 1035) now show orange "Cash" or green "Paid" based on cashAppointmentIds
